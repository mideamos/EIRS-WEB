$(document).ready(function () {
    ...
    var vfrmTreasuryReceiptByRevenueStream = $("#frmTreasuryReceiptByRevenueStream");
    vfrmTreasuryReceiptByRevenueStream.validate({
    ...
    $("#btnSearch").click(function () {
        if (vfrmTreasuryReceiptByRevenueStream.valid()) {
            $('#dvSearchData').show();
            jsfn_bindTable();
            return false;
        }
    ...
    $("#btnExportToExcel").click(function () {
        ...
        var vUrl = '/OperationManager/TreasuryReceiptByRevenueStreamExportToExcel?TaxYear=' + vYear + '&FromDate=' + vFromDate + '&ToDate=' + vToDate;
...
var vTreasuryReceiptByRevenueStream;
function jsfn_bindTable() {
    if (vTreasuryReceiptByRevenueStream !== undefined) {
        vTreasuryReceiptByRevenueStream.draw();
    }
    else {
        vTreasuryReceiptByRevenueStream = $("#tblTreasuryReceiptByRevenueStream").DataTable({
            "processing": true, // for show progress bar
            "serverSide": true, // for process server side
            "filter": true,
            "orderMulti": false, // for disable multiple column at once
            "ajax": {
                "url": "/OperationManager/TreasuryReceiptByRevenueStreamLoadData",
                "type": "POST",
                "datatype": "json",
                "data": function (data) {
                ...
                { "data": "TransactionCount", "orderable": true, "name": "TransactionCount" },
                {
                    "data": "", "orderable": false, "name": "Action", "render": function (data, type, st) {
                        return '<div class="btn-group"><button type = "button" class= "btn btn-theme dropdown-toggle md-skip btn-xs" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">Action<span class="caret"></span>'
                            ...
                            + '<li><a href="/OperationManager/TreasuryReceiptByRevenueStreamDetail?tyear=' + $("#cboYear").val() + '&rstrmID=' + st.RevenueStreamID + '&fdate=' + $('#txtFromDate').val() + '&tdate=' + $('#txtToDate').val() + '">View Details</a></li>'
                            + '<li><a href="/OperationManager/TreasuryReceiptByRevenueStreamDetailExportToExcel?TaxYear=' + $("#cboYear").val() + '&RevenueStreamID=' + st.RevenueStreamID + '&FromDate=' + $('#txtFromDate').val() + '&ToDate=' + $('#txtToDate').val() + '">Export Details</a></li>'
                            + '</ul></div>';
                    }, "width": "10%"
                },
            ],
