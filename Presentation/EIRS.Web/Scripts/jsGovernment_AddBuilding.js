$(document).ready(function () {
    ...
    $("#btnSaveBuildingUnit").click(function () {
        if (vfrmBuildingUnit.valid()) {
            ...
            jsfn_ShowLoading();
            jsfn_ajaxPost('/CaptureGovernment/AddBuildingUnit', vData, jsfn_onAddBuildingResponse);
        }
    });

    $('#btnSaveBuilding').click(function () {
        if (parseInt($("#hdnBuildingUnitID").val()) > 0) {
            $("#frmAddBuilding").submit();
        }
        else {
            jsfn_ShowAlert('Select or Add Atleast One Building Unit', 'danger');
        }
...
function jsfn_onAddBuildingResponse(data) {
    ...
    if (data.success) {
        ...
        $(".icheck").on("ifChecked", function () {
            jsfn_onBuildingUnitSelected($(this).data("rowid").toString());
        });
    }
    else {
...
function jsfn_onBuildingUnitSelected(rowid) {
    $('#hdnBuildingUnitID').val(rowid);
}
...
function jsfn_onTownChange() {
    ...
    if (vTownID != '') {
        ...
        jsfn_ajaxPost('/CaptureGovernment/GetTownDetails', vData, jsfn_onTownChangeResponse);
    }
}
...
function jsfn_onLGAChange() {
    ...
    jsfn_ajaxPost('/CaptureGovernment/GetWard', vData, jsfn_onLGAChangeResponse);
}
...
function jsfn_onUnitPurposeChange() {
    ...
    jsfn_ajaxPost('/CaptureGovernment/GetUnitFunction', vData, jsfn_onUnitPurposeChangeResponse);
}
