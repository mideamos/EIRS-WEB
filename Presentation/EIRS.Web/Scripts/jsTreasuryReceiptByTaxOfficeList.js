...
$(document).ready(function () {
    ...
    var vfrmTreasuryReceiptByTaxOffice = $("#frmTreasuryReceiptByTaxOffice");
    vfrmTreasuryReceiptByTaxOffice.validate({
    ...
    $("#btnSearch").click(function () {
        if (vfrmTreasuryReceiptByTaxOffice.valid()) {
            $('#dvSearchData').show();
            jsfn_bindTable();
            return false;
        }
    ...
    $("#btnExportToExcel").click(function () {
        ...
        var vUrl = '/OperationManager/TreasuryReceiptByTaxOfficeExportToExcel?TaxYear=' + vYear + '&FromDate=' + vFromDate + '&ToDate=' + vToDate;
...
var vTreasuryReceiptByTaxOffice;
function jsfn_bindTable() {
    if (vTreasuryReceiptByTaxOffice !== undefined) {
        vTreasuryReceiptByTaxOffice.draw();
    }
    else {
        vTreasuryReceiptByTaxOffice = $("#tblTreasuryReceiptByTaxOffice").DataTable({
            "processing": true, // for show progress bar
            "serverSide": true, // for process server side
            "filter": true,
            "orderMulti": false, // for disable multiple column at once
            "ajax": {
                "url": "/OperationManager/TreasuryReceiptByTaxOfficeLoadData",
                "type": "POST",
                "datatype": "json",
                "data": function (data) {
                ...
                { "data": "TransactionCount", "orderable": true, "name": "TransactionCount" },
                {
                    "data": "", "orderable": false, "name": "Action", "render": function (data, type, st) {
                        return '<div class="btn-group"><button type = "button" class= "btn btn-theme dropdown-toggle md-skip btn-xs" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">Action<span class="caret"></span>'
                            ...
                            + '<li><a href="/OperationManager/TreasuryReceiptByTaxOfficeDetail?tyear=' + $("#cboYear").val() + '&toffID=' + st.TaxOfficeID + '&fdate=' + $('#txtFromDate').val() + '&tdate=' + $('#txtToDate').val() + '">View Details</a></li>'
                            + '<li><a href="/OperationManager/TreasuryReceiptByTaxOfficeDetailExportToExcel?TaxYear=' + $("#cboYear").val() + '&TaxOfficeID=' + st.TaxOfficeID + '&FromDate=' + $('#txtFromDate').val() + '&ToDate=' + $('#txtToDate').val() + '">Export Details</a></li>'
                            + '</ul></div>';
                    }, "width": "10%"
                },
            ],
