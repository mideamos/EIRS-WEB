//QC_VP_T-EARS-56/63/64_OperationManagerMakeTCCMonthlySummary
$(document).ready(function () {
    $('#dvSearchData').hide();

    var vfrmTCCMonthlySummary = $("#frmTCCMonthlySummary");
    vfrmTCCMonthlySummary.validate({
    ...
    $("#btnSearch").click(function () {
        if (vfrmTCCMonthlySummary.valid()) {
            $('#dvSearchData').show();
            jsfn_bindTable();
            return false;
        }
    ...
    $("#btnExportToExcel").click(function () {
        ...
        var vUrl = '/OperationManager/TCCMonthlySummaryExportToExcel?StatusID=' + vStatusID + '&TaxYear=' + vYear;
...
var vTCCMonthlySummary;
function jsfn_bindTable() {
    if (vTCCMonthlySummary !== undefined) {
        vTCCMonthlySummary.draw();
    }
    else {
        vTCCMonthlySummary = $("#tblTCCMonthlySummary").DataTable({
            "processing": true, // for show progress bar
            "serverSide": true, // for process server side
            "filter": true,
            "orderMulti": false, // for disable multiple column at once
            "ajax": {
                "url": "/OperationManager/TCCMonthlySummaryLoadData",
                "type": "POST",
                "datatype": "json",
                "data": function (data) {
                ...
                { "data": "RequestCount", "orderable": true, "name": "RequestCount" },
                {
                    "data": "", "orderable": false, "name": "Action", "render": function (data, type, st) {
                        return '<div class="btn-group"><button type = "button" class= "btn btn-theme dropdown-toggle md-skip btn-xs" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">Action<span class="caret"></span>'
                            ...
                            + '<li><a href="/OperationManager/TCCMonthlySummaryDetail?year=' + $("#cboYear").val() + '&month=' + st.MonthID + '&statid=' + ($("#cboStatus").val() !== "" ? $("#cboStatus").val() : 0) + '">View Details</a></li>'
                            + '<li><a href="/OperationManager/TCCMonthlySummaryDetailExportToExcel?TaxYear=' + $("#cboYear").val() + '&TaxMonth=' + st.MonthID + '&StatusID=' + ($("#cboStatus").val() !== "" ? $("#cboStatus").val() : 0) + '">Export Details</a></li>'
                            + '</ul></div>';
                    }, "width": "10%"
                },
            ],
