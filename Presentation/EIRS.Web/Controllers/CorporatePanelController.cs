    ...
    public class CorporatePanelController : BaseController
    {
        ...
        public ActionResult Edit()
        {
            if (SessionManager.TaxPayerID > 0)
            {
                ...
                {
                    CompanyID = SessionManager.TaxPayerID,
                    intStatus = 1
                };
            ...
            else
            {
                ...
                try
                {
                    ...
                    if (mObjResponse.Success)
                    {
                        ...
                        return RedirectToAction("Details", "CorporatePanel");
                    }
                    else
                    {
                        UI_FillDropDown(pObjCompanyModel);
        ...
        public ActionResult Details()
        {
            if (SessionManager.TaxPayerID > 0)
            {
                ...
                if (mObjCompanyData != null)
                {
                    ...
                    IList<usp_GetAssessmentRuleInformation_Result> lstAssessmentRuleInformation = new BLTaxPayerAsset().BL_GetTaxPayerAssessmentRuleInformation((int)EnumList.TaxPayerType.Companies, SessionManager.TaxPayerID);
                    ViewBag.AssessmentRuleInformation = lstAssessmentRuleInformation;

                    IList<usp_GetTaxPayerBill_Result> lstTaxPayerBill = new BLAssessment().BL_GetTaxPayerBill(SessionManager.TaxPayerID, (int)EnumList.TaxPayerType.Companies, 0);
                    ViewBag.TaxPayerBill = lstTaxPayerBill;

                    IList<usp_GetTaxPayerPayment_Result> lstTaxPayerPayment = new BLSettlement().BL_GetTaxPayerPayment(SessionManager.TaxPayerID, (int)EnumList.TaxPayerType.Companies);
                    ViewBag.TaxPayerPayment = lstTaxPayerPayment;

                    IList<usp_GetTaxPayerMDAService_Result> lstMDAService = new BLMDAService().BL_GetTaxPayerMDAService((int)EnumList.TaxPayerType.Companies, SessionManager.TaxPayerID);
                    ...
                    try
                    {
                        ...
                        if (mObjResponse.Success)
                        {
                            ...
                            if (mObjTPResponse.Success)
                            {
                                ...
                                return RedirectToAction("Details", "CorporatePanel", new { id = pObjBusinessModel.TaxPayerID, name = pObjBusinessModel.TaxPayerRIN });
                            }
                            else
                            {
                    ...
                    try
                    {
                        ...
                        if (mObjResponse.Success)
                        {
                            ...
                            if (mObjTPResponse.Success)
                            {
                                ...
                                return RedirectToAction("Details", "CorporatePanel", new { id = pObjLandModel.TaxPayerID, name = pObjLandModel.TaxPayerRIN });
                            }
                            else
                            {
                                throw new Exception(mObjResponse.Message);
                    ...
                    try
                    {
                        ...
                        if (mObjResponse.Success)
                        {
                            ...
                            if (mObjTPResponse.Success)
                            {
                                ...
                                return RedirectToAction("Details", "CorporatePanel", new { id = pObjVehicleModel.TaxPayerID, name = pObjVehicleModel.TaxPayerRIN });
                            }
                            else
                            {
                                throw new Exception(mObjResponse.Message);
                        ...
                        try
                        {
                            ...
                            if (mObjResponse.Success)
                            {
                                ...
                                if (mObjTPResponse.Success)
                                {
                                    ...
                                    return RedirectToAction("Details", "CorporatePanel", new { id = pObjBuildingModel.TaxPayerID, name = pObjBuildingModel.TaxPayerRIN });
                                }
                                else
                                {
        ...
        public ActionResult GenerateServiceBill()
        {
            if (SessionManager.TaxPayerID > 0)
            {
                ...
                if (mObjCompanyData != null)
                {
                    ...
                    //IList<usp_GetMDAServiceForServiceBill_Result> lstMDAService = new BLMDAService().BL_GetMDAServiceForServiceBill((int)EnumList.TaxPayerType.Companies, SessionManager.TaxPayerID);
                    ...
                    else
                    {
                        ...
                        try
                        {
                            ...
                            if (mObjServiceBillResponse.Success)
                            {
                                ...
                                return RedirectToAction("Details", "CorporatePanel", new { id = pObjServiceBillModel.TaxPayerID, name = pObjServiceBillModel.TaxPayerRIN });
                            }
                            else
                            {
                                ViewBag.MDAServiceList = SessionManager.lstMDAService.Where(t => t.intTrack != EnumList.Track.DELETE).ToList();
