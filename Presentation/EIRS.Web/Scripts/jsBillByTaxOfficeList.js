////QC_VP_T-ERAS-65_BillByTaxOfficeAndBillDetailsByTaxOfficeAndExport_01
$(document).ready(function () {
    ...
    var vfrmBillByTaxOffice = $("#frmBillByTaxOffice");
    vfrmBillByTaxOffice.validate({
    ...
    $("#btnExportToExcel").click(function () {
        ...
        var vUrl = '/OperationManager/BillByTaxOfficeExportToExcel?BillTypeID=' + vBillTypeID + '&BillStatusID=' + vBillStatusID + '&TaxYear=' + vYear + '&FromDate=' + vFromDate + '&ToDate=' + vToDate;
...
var vBillByTaxOffice;
function jsfn_bindTable() {

    if (vBillByTaxOffice !== undefined) {
        vBillByTaxOffice.draw();
    }
    else {
        vBillByTaxOffice = $("#tblBillByTaxOffice").DataTable({
            "processing": true, // for show progress bar
            "serverSide": true, // for process server side
            "filter": true,
            "orderMulti": false, // for disable multiple column at once
            "ajax": {
                "url": "/OperationManager/BillByTaxOfficeLoadData",
                "type": "POST",
                "datatype": "json",
                "data": function (data) {
                ...
                { "data": "TransactionCount", "orderable": true, "name": "TransactionCount" },
                {
                    "data": "", "orderable": false, "name": "Action", "render": function (data, type, st) {
                        return '<div class="btn-group"><button type = "button" class= "btn btn-theme dropdown-toggle md-skip btn-xs" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">Action<span class="caret"></span>'
                            ...
                            + '<li><a href="/OperationManager/BillByTaxOfficeDetail?tyear=' + $("#cboYear").val() + '&toffID=' + st.TaxOfficeID + '&fdate=' + $("#txtFromDate").val() + '&tdate=' + $("#txtToDate").val() + '&btypeID=' + $("#cboBillType").val() + '&bstatID=' + $("#cboBillStatus").val() + '">View Details</a></li>'
                            + '<li><a target="_blank" href="/OperationManager/BillByTaxOfficeDetailExportToExcel?TaxYear=' + $("#cboYear").val() + '&TaxOfficeID=' + st.TaxOfficeID + '&FromDate=' + $("#txtFromDate").val() + '&ToDate=' + $("#txtToDate").val() + '&BillTypeID=' + $("#cboBillType").val() + '&BillStatusID=' + $("#cboBillStatus").val() + '">Export Details</a></li>'
                            + '</ul></div>';
                    }, "width": "10%"
                },
            ],
