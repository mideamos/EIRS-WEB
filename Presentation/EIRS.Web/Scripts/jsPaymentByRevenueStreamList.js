//QC_VP_T-EARS-56/63/64_OperationManagerMakePaymentByRevenueStream
$(document).ready(function () {
    ...
    var vfrmPaymentByRevenueStream = $("#frmPaymentByRevenueStream");
    vfrmPaymentByRevenueStream.validate({
    ...
    $("#btnSearch").click(function () {
        if (vfrmPaymentByRevenueStream.valid()) {
            $('#dvSearchData').show();
            jsfn_bindTable();
            return false;
        }
    ...
    $("#btnExportToExcel").click(function () {
        ...
        var vUrl = '/OperationManager/PaymentByRevenueStreamExportToExcel?PaymentTypeID=' + vPaymentTypeID + '&TaxYear=' + vYear + '&FromDate=' + vFromDate + '&ToDate=' + vToDate;
...
var vPaymentByRevenueStream;
function jsfn_bindTable() {
    if (vPaymentByRevenueStream !== undefined) {
        vPaymentByRevenueStream.draw();
    }
    else {
        vPaymentByRevenueStream = $("#tblPaymentByRevenueStream").DataTable({
            "processing": true, // for show progress bar
            "serverSide": true, // for process server side
            "filter": true,
            "orderMulti": false, // for disable multiple column at once
            "ajax": {
                "url": "/OperationManager/PaymentByRevenueStreamLoadData",
                "type": "POST",
                "datatype": "json",
                "data": function (data) {
                ...
                { "data": "TransactionCount", "orderable": true, "name": "TransactionCount" },
                {
                    "data": "", "orderable": false, "name": "Action", "render": function (data, type, st) {
                        return '<div class="btn-group"><button type = "button" class= "btn btn-theme dropdown-toggle md-skip btn-xs" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">Action<span class="caret"></span>'
                            ...
                            + '<li><a href="/OperationManager/PaymentByRevenueStreamDetail?ptypeId=' + ($("#cboPaymentType").val() !== '' ? $("#cboPaymentType").val() : 0) + '&tyear=' + $("#cboYear").val() + '&rstrmID=' + st.RevenueStreamID + '&fdate=' + $('#txtFromDate').val() + '&tdate=' + $('#txtToDate').val() + '">View Details</a></li>'
                            + '<li><a href="/OperationManager/PaymentByRevenueStreamDetailExportToExcel?PaymentTypeID=' + ($("#cboPaymentType").val() !== '' ? $("#cboPaymentType").val() : 0) + '&TaxYear=' + $("#cboYear").val() + '&RevenueStreamID=' + st.RevenueStreamID + '&FromDate=' + $('#txtFromDate').val() + '&ToDate=' + $('#txtToDate').val() + '">Export Details</a></li>'
                            + '</ul></div>';
                    }, "width": "10%"
                },
            ],
