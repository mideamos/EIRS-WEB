$(document).ready(function () {
    ...
    var frmGenerateServiceBill = $('#frmGenerateServiceBill');

    // init validator obj and set the rules
    frmGenerateServiceBill.validate({
    ...
    $("#btnBack").click(function () {
        ...
        $("#btnGenerate").hide();
        $("#btnBack").hide();
        $("#artGenerateServiceBill").addClass("col-lg-6");
        return false;
    });

    $('#btnGenerate').click(function () {
        if ($("#hdnTaxPayerId").val() != '') {
            window.location.href = '/servicebill/add/' + $("#hdnTaxPayerId").val() + '/' + $("#hdnTaxPayerRIN").val() + '?tptype=' + $("#cboTaxPayerType").val();
        }
        else {
...
function jsfn_BindTaxPayerTable() {
    ...
    jsfn_ShowLoading();
    jsfn_ajaxPost('/ServiceBill/GetTaxPayerList', vData, jsfn_BindTaxPayerTableResponse);
}
...
function jsfn_BindTaxPayerTableResponse(data) {
    ...
    if (data.success) {
        ...
        $("#artGenerateServiceBill").removeClass("col-lg-6");
