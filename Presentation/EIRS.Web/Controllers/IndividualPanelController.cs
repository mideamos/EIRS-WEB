    ...
    public class IndividualPanelController : BaseController
    {
        ...
        public ActionResult Edit(IndividualViewModel pObjIndividualModel)
        {
            ...
            else
            {
                Individual mObjIndividual = new Individual()
                {
                    ...
                    Active = true,
                    ModifiedBy = SessionManager.TaxPayerID,
                ...
                try
                {
                    ...
                    if (mObjResponse.Success)
                    {
                        ...
                        return RedirectToAction("Details", "IndividualPanel");
                    }
                    else
                    {
                        UI_FillDropDown(pObjIndividualModel);
        ...
        public ActionResult Details()
        {
            ...
            if (mObjIndividualData != null)
            {
                ...
                IList<usp_GetAssessmentRuleInformation_Result> lstAssessmentRuleInformation = new BLTaxPayerAsset().BL_GetTaxPayerAssessmentRuleInformation((int)EnumList.TaxPayerType.Individual, SessionManager.TaxPayerID);
                ViewBag.AssessmentRuleInformation = lstAssessmentRuleInformation;

                IList<usp_GetTaxPayerBill_Result> lstTaxPayerBill = new BLAssessment().BL_GetTaxPayerBill(SessionManager.TaxPayerID, (int)EnumList.TaxPayerType.Individual, 0);
                ViewBag.TaxPayerBill = lstTaxPayerBill;

                IList<usp_GetTaxPayerPayment_Result> lstTaxPayerPayment = new BLSettlement().BL_GetTaxPayerPayment(SessionManager.TaxPayerID, (int)EnumList.TaxPayerType.Individual);
                ViewBag.TaxPayerPayment = lstTaxPayerPayment;

                IList<usp_GetTaxPayerMDAService_Result> lstMDAService = new BLMDAService().BL_GetTaxPayerMDAService((int)EnumList.TaxPayerType.Individual, SessionManager.TaxPayerID);
                    ...
                    try
                    {
                        ...
                        if (mObjResponse.Success)
                        {
                            ...
                            if (mObjTPResponse.Success)
                            {
                                ...
                                return RedirectToAction("Details", "IndividualPanel", new { id = pObjBusinessModel.TaxPayerID, name = pObjBusinessModel.TaxPayerRIN });
                            }
                            else
                            {
                    ...
                    try
                    {
                        ...
                        if (mObjResponse.Success)
                        {
                            ...
                            if (mObjTPResponse.Success)
                            {
                                ...
                                return RedirectToAction("Details", "IndividualPanel", new { id = pObjLandModel.TaxPayerID, name = pObjLandModel.TaxPayerRIN });
                            }
                            else
                            {
                                throw new Exception(mObjResponse.Message);
                    ...
                    try
                    {
                        ...
                        if (mObjResponse.Success)
                        {
                            ...
                            if (mObjTPResponse.Success)
                            {
                                ...
                                return RedirectToAction("Details", "IndividualPanel", new { id = pObjVehicleModel.TaxPayerID, name = pObjVehicleModel.TaxPayerRIN });
                            }
                            else
                            {
                                throw new Exception(mObjResponse.Message);
                        ...
                        try
                        {
                            ...
                            if (mObjResponse.Success)
                            {
                                ...
                                if (mObjTPResponse.Success)
                                {
                                    ...
                                    return RedirectToAction("Details", "IndividualPanel", new { id = pObjBuildingModel.TaxPayerID, name = pObjBuildingModel.TaxPayerRIN });
                                }
                                else
                                {
        ...
        public ActionResult GenerateServiceBill()
        {
            if (SessionManager.TaxPayerID > 0)
            {
                ...
                if (mObjIndividualData != null)
                {
                    ...
                    //IList<usp_GetMDAServiceForServiceBill_Result> lstMDAService = new BLMDAService().BL_GetMDAServiceForServiceBill((int)EnumList.TaxPayerType.Individual, SessionManager.TaxPayerID);
                    ...
                    else
                    {
                        ...
                        try
                        {
                            ...
                            if (mObjServiceBillResponse.Success)
                            {
                                ...
                                return RedirectToAction("Details", "IndividualPanel", new { id = pObjServiceBillModel.TaxPayerID, name = pObjServiceBillModel.TaxPayerRIN });
                            }
                            else
                            {
                                ViewBag.MDAServiceList = SessionManager.lstMDAService.Where(t => t.intTrack != EnumList.Track.DELETE).ToList();
        ...
        public ActionResult EditServiceBill(int? sbid)
        {
            if (SessionManager.TaxPayerID > 0 && sbid.GetValueOrDefault() > 0)
            {
                ...
                if (mObjIndividualData != null && mObjServiceBillData != null && mObjServiceBillData.TaxPayerID == SessionManager.TaxPayerID)
                {
                    ServiceBillViewModel mObjServiceBillModel = new ServiceBillViewModel()
                    {
                        TaxPayerID = mObjIndividualData.IndividualID.GetValueOrDefault(),
                ...
                if (mObjIndividualData != null)
                {
                    if (billrefno.StartsWith("SB"))
                    {
                        ...
                        if (mObjAssessmentData != null && mObjAssessmentData.TaxPayerID == mObjIndividualData.IndividualID && mObjAssessmentData.TaxPayerTypeID == (int)EnumList.TaxPayerType.Individual)
                        {
                            ...
                            Rpt[3] = new ReportParameter("rptTaxPayerRIN", mObjIndividualData.IndividualRIN);
                            Rpt[4] = new ReportParameter("rptTaxPayerName", mObjIndividualData.FirstName + " " + mObjIndividualData.LastName);
                            Rpt[5] = new ReportParameter("rptTaxPayerNumber", mObjIndividualData.MobileNumber1);
                            Rpt[6] = new ReportParameter("rptTaxPayerContactAddress", mObjIndividualData.ContactAddress);
